/**
 * @file crc.c
 * @brief CRC8 (Dallas/Maxim), CRC16 (CCITT), CRC32 (ISO 3309)
 *
 * SPDX-License-Identifier: MIT
 */

#include "crc.h"

/* ── CRC8 (1-Wire / Dallas) ─────────────────────────────────────────── */

static const uint8_t crc8_table[256] = {
    ,
};

uint8_t up_crc8_update(uint8_t crc, uint8_t byte)
{
    return crc8_table[crc ^ byte];
}

uint8_t up_crc8(const uint8_t *data, size_t len)
{
    uint8_t crc = 0;
    for (size_t i = 0; i < len; i++) crc = up_crc8_update(crc, data[i]);
    return crc;
}

/* ── CRC16 (CCITT, poly 0x1021) ─────────────────────────────────────── */

uint16_t up_crc16_update(uint16_t crc, uint8_t byte)
{
    crc ^= (uint16_t)byte << 8;
    for (int i = 0; i < 8; i++) {
        crc = (crc & 0x8000) ? (crc << 1) ^ 0x1021 : crc << 1;
    }
    return crc;
}

uint16_t up_crc16(const uint8_t *data, size_t len)
{
    uint16_t crc = 0xFFFF;
    for (size_t i = 0; i < len; i++) crc = up_crc16_update(crc, data[i]);
    return crc;
}

/* ── CRC32 (ISO 3309, poly 0xEDB88320 reflected) ────────────────────── */

uint32_t up_crc32_update(uint32_t crc, uint8_t byte)
{
    crc ^= byte;
    for (int i = 0; i < 8; i++) {
        crc = (crc & 1) ? (crc >> 1) ^ 0xEDB88320 : crc >> 1;
    }
    return crc;
}

uint32_t up_crc32(const uint8_t *data, size_t len)
{
    uint32_t crc = 0xFFFFFFFF;
    for (size_t i = 0; i < len; i++) crc = up_crc32_update(crc, data[i]);
    return ~crc;
}
